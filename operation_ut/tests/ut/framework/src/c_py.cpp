...
int PyScripts::Initialize(const string &full_path) {
    ...
    string onnx_module = "gen_data_from_onnx";
    ...
    PyOnnxModule = PyHolder::GetInstance().ImportModule(onnx_module.c_str());
    if (PyOnnxModule == nullptr) {
        return 1;
    }
    ...
    PyOnnxFunc = PyHolder::GetInstance().ImportFunction(PyOnnxModule, "main");
    if (PyOnnxFunc == nullptr) {
        return 1;
    }
    return 0;
}
...
void PyScripts::Clean()
{
    ...
    if (PyOnnxFunc != nullptr) {
        PyHolder::GetInstance().Ut_Py_Dealloc(PyOnnxFunc);
    }
    if (PyInputModule != nullptr) {
    ...
    if (PyOnnxModule != nullptr) {
        PyHolder::GetInstance().Ut_Py_Dealloc(PyOnnxModule);
    }
}
...
int PyScripts::GenDataFromOnnx(const string &case_onnx, const string &case_json) const
{
    unique_ptr<PyObject, decltype(&PyObjectDecRef)> pArgs(PyHolder::GetInstance().Ut_PyTuple_New(2), PyObjectDecRef);
    PyHolder::GetInstance().Ut_PyTuple_SetItem(pArgs.get(), 0,
                                               PyHolder::GetInstance().Ut_PyBuildValue("s", case_onnx.c_str()));
    PyHolder::GetInstance().Ut_PyTuple_SetItem(pArgs.get(), 1,
                                               PyHolder::GetInstance().Ut_PyBuildValue("s", case_json.c_str()));
    return CallFunction(PyOnnxFunc, pArgs.get());
}
